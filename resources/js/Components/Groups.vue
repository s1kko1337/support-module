...
<template>
    <div class="bg-gradient-to-br from-indigo-50 via-white to-blue-50 min-h-screen pb-12">
            ...
            <div class="bg-white rounded-2xl shadow-xl overflow-hidden mb-8 transform transition duration-300 hover:shadow-2xl">
                ...
                <div class="p-6 md:p-8">
                    <div class="flex flex-col md:flex-row justify-between items-center mb-6">
                        <h2 class="text-2xl font-bold text-gray-800 mb-4 md:mb-0">Список учебных групп</h2>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full bg-white rounded-xl overflow-hidden">
                            ...
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Кол-во студентов</th>
                            ...
                            <tr v-for="group in groups" :key="group.id" class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                    <button @click="viewGroup(group)" class="text-indigo-600 hover:text-indigo-900 p-1 rounded-full hover:bg-indigo-50">
                                        <Eye class="w-5 h-5" />
                                    </button>
                                    <button @click="editGroup(group)" class="text-amber-600 hover:text-amber-900 p-1 rounded-full hover:bg-amber-50">
                                        <Edit class="w-5 h-5" />
                                    </button>
                                    <button @click="confirmDelete(group)" class="text-red-600 hover:text-red-900 p-1 rounded-full hover:bg-red-50">
        ...
        <!-- Модальное окно для просмотра группы -->
            ...
            <div class="modal-content animate-scaleIn max-w-lg">
                ...
                <div class="p-6">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-xl font-bold text-gray-800">Просмотр группы</h3>
                    ...
                    <div class="space-y-6">
                        <div class="bg-indigo-50 p-4 rounded-lg">
                            <span class="text-sm text-indigo-700 font-medium">Название группы</span>
                            <div class="text-lg font-semibold mt-1">{{ viewModal.group.name }}</div>
                        </div>

                        <div class="bg-blue-50 p-4 rounded-lg">
                            <span class="text-sm text-blue-700 font-medium">Количество студентов</span>
                            <div class="text-2xl font-semibold mt-1">{{ viewModal.group.number_of_students }}</div>
                        </div>

                        <div v-if="viewModal.group.curator_id" class="bg-green-50 p-4 rounded-lg">
                            <span class="text-sm text-green-700 font-medium">ID Куратора</span>
                            <div class="text-lg font-semibold mt-1">{{ viewModal.group.curator_id }}</div>
                        </div>

                        <div class="grid grid-cols-2 gap-4">
                            <div class="p-4 border border-gray-200 rounded-lg">
                                <span class="text-sm text-gray-500 font-medium">Дата создания</span>
                                <div class="text-md font-medium mt-1">{{ formatDate(viewModal.group.created_at) }}</div>
                            </div>

                            <div class="p-4 border border-gray-200 rounded-lg">
                                <span class="text-sm text-gray-500 font-medium">Последнее обновление</span>
                                <div class="text-md font-medium mt-1">{{ formatDate(viewModal.group.updated_at) }}</div>
                            </div>
                        </div>

                        <div class="flex justify-end pt-4">
                            <button @click="editGroup(viewModal.group)" class="btn-primary mr-3">
                ...
                <div class="p-6">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-xl font-bold text-gray-800">
                            {{ formModal.isEdit ? 'Редактирование группы' : 'Добавление группы' }}
                        </h3>
                    ...
                    <form @submit.prevent="saveGroup" class="space-y-4">
                        <!-- Название группы -->
                        <div>
                            <label for="group-name" class="block text-sm font-medium text-gray-700 mb-1">Название группы*</label>
                            <input
                                id="group-name"
                        ...
                        <div>
                            <label for="number-of-students" class="block text-sm font-medium text-gray-700 mb-1">Количество студентов*</label>
                            <input
                                id="number-of-students"
                                v-model="formModal.form.number_of_students"
                                type="number"
                                min="0"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                ...
                                required
                            >
                            <span v-if="formErrors.number_of_students" class="text-sm text-red-600">{{ formErrors.number_of_students[0] }}</span>
                        </div>
                ...
                <div class="p-6">
                    <div class="flex flex-col items-center mb-6 text-center">
                        ...
                        <p class="text-gray-600">
                            Вы уверены, что хотите удалить группу "{{ deleteModal.group.name }}"?
...
// Открытие модального окна для просмотра группы
const viewGroup = async (group) => {
    try {
        // Получаем полные данные о группе
        const response = await axios.get(`/api/v1/groups/${group.id}`);
        viewModal.group = response.data;
        viewModal.show = true;
    } catch (error) {
        showToast('error', 'Ошибка', 'Не удалось загрузить данные группы.');
...
const saveGroup = async () => {
    ...
    try {
        ...
        if (formModal.isEdit) {
            // Обновление группы
            response = await axios.put(`/api/v1/groups/${formModal.groupId}`, formModal.form);
            showToast('success', 'Группа обновлена', 'Информация о группе успешно обновлена.');
        } else {
            // Создание новой группы
            response = await axios.post('/api/v1/groups', formModal.form);
            showToast('success', 'Группа создана', 'Новая группа успешно добавлена.');
        }
        ...
        if (error.response && error.response.data && error.response.data.errors) {
            ...
            showToast('error', 'Ошибка', 'Произошла ошибка при сохранении группы.');
        }
...
const deleteGroup = async () => {
    ...
    try {
        await axios.delete(`/api/v1/groups/${deleteModal.group.id}`);
        showToast('success', 'Группа удалена', 'Группа успешно удалена из системы.');
        ...
        showToast('error', 'Ошибка удаления', 'Не удалось удалить группу.');
